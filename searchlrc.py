...
def download1(path, song_singer):
    ...
    try:
        url1 = 'http://music.163.com/api/song/lyric?os=pc&id=' + str(
            search_rs['result']['songs'][0]['id']) + '&lv=-1&kv=-1&tv=-1'
        lrc = urllib2.Request(
            url=url1,
            headers={'referer': 'http://music.163.com', 'cookies': 'appver=2.0.2;'},
        )
        lrc_rs = json.loads(opener.open(lrc).read())
        if len(lrc_rs['lrc']['lyric'].encode('UTF-8')) != 0:
            try:
...
if __name__ == '__main__':
    ...
    # path = r'H:\MUSIC\鏋椾繆鍌? 	#姝屾洸璺緞
    # uipath = unicode(path,"utf8")
uipath = r'E:\MUSIC'
for i in scan1(uipath):
    try:
        if os.path.exists(i[:-5] + '.lrc') == False:
